...
public class GitLabClonerServiceImplTests {
  ...
  @BeforeEach
  public void setUp() {
    Mockito.when(gitCommandsHelper.getGitCloneCommand(ArgumentMatchers.anyString())).thenReturn("ls");

    Mockito.when(systemUtilsHelper.isOsWindows()).thenReturn(SystemUtils.IS_OS_WINDOWS);
    Mockito.when(systemUtilsHelper.getUserHome()).thenReturn(System.getProperty("user.home").concat("/"));
    ...
    Mockito.when(gitLab4JApiRepository.getAllProjectsByGroup(ArgumentMatchers.anyString()))
        .thenReturn(DummyDataFactory.getFakedProjectsForGroup(TEST_GROUP));
  ...
  public void cloneProjectWithNullDirectoryThrowsNullPointerException() {
    ...
    final GitLabClonerServiceImpl gitLabClonerServiceImpl = new GitLabClonerServiceImpl(gitLab4JApiRepository, gitCommandsHelper, systemUtilsHelper, executor);
  ...
  public void cloneProjectHttpsWithExecutorException() {
    ...
    Mockito.doThrow(RuntimeException.class).when(executor).execute(ArgumentMatchers.any(Runnable.class));
  ...
  public void cloneProjectSshWithFakeCommandAndRealExecutorThrowsGitLabServiceProcessException() {
    ...
    Mockito.when(gitCommandsHelper.getGitCloneCommand(ArgumentMatchers.anyString())).thenReturn("fake_command");

    Executor realExecutor = Executors.newSingleThreadExecutor();

    final GitLabClonerServiceImpl gitLabClonerServiceImpl = new GitLabClonerServiceImpl(gitLab4JApiRepository, gitCommandsHelper, systemUtilsHelper,
        realExecutor);
